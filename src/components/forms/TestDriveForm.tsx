...
export default function TestDriveForm({ modelNames }: TestDriveFormProps) {
  ...
  if (status === 'success') {
    return (
      <div className="text-center py-8">
        ...
        <h3 className="text-xl font-bold text-text-primary mb-2">Test Drive Booked!</h3>
        <p className="text-text-muted">We&apos;ll confirm your appointment shortly. Get ready to experience Suzuki!</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        <label className="block text-sm font-medium text-text-muted mb-1.5">Which model would you like to test drive? *</label>
        <select
          required
          value={form.model}
          onChange={(e) => setForm({ ...form, model: e.target.value })}
          className="w-full px-3 py-2.5 rounded-lg border border-gray-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-suzuki-teal/30 focus:border-suzuki-teal"
        >
          <option value="">Select a model...</option>
          {modelNames.map((name) => (
            <option key={name} value={name}>Suzuki {name}</option>
          ))}
        </select>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-text-muted mb-1.5">Preferred Date</label>
          <input type="date" value={form.preferredDate} onChange={(e) => setForm({ ...form, preferredDate: e.target.value })} className="w-full px-3 py-2.5 rounded-lg border border-gray-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-suzuki-teal/30 focus:border-suzuki-teal" />
        </div>
        <div>
          <label className="block text-sm font-medium text-text-muted mb-1.5">Preferred Time</label>
          <select value={form.preferredTime} onChange={(e) => setForm({ ...form, preferredTime: e.target.value })} className="w-full px-3 py-2.5 rounded-lg border border-gray-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-suzuki-teal/30 focus:border-suzuki-teal">
      ...
      <div>
        <label className="block text-sm font-medium text-text-muted mb-1.5">Anything else we should know?</label>
      ...
      >
        {status === 'loading' ? 'Booking...' : 'Book My Test Drive'}
      </button>
    </form>
  );
}
